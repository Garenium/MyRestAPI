<template>
  <q-page>
    <div class="text-center q-mt-lg">
      ...
      <img src="/videogamestore.png" width="250" height="255" />
    ...
    >
      <q-card>
        ...
        <q-card-section class="q-pa-none text-center">
          ...
          <div style="font-weight: bold">
            {{ formatCurrency(state.selectedProduct.costPrice) }}
          </div>
        </q-card-section>
  ...
  setup() {
    ...
    const addToCart = () => {
      ...
      if (state.qty > 0) {
        ...
        state.dialogStatus = `${state.qty} products(s) added`;
      } else {
        index === -1 ? null : state.cart.splice(index, 1); //remove
        state.dialogStatus = `products(s) removed`;
      }
